...
func addGinkgoImports(rootNode *ast.File) {
	...
	for _, importSpec := range importDecl.Specs {
		...
		if importSpec.Path.Value == "\"github.com/yyleeshine/mpquic/repository/onsi/ginkgo\"" {
			needsGinkgo = false
		}
	}

	if needsGinkgo {
		importDecl.Specs = append(importDecl.Specs, createImport(".", "\"github.com/yyleeshine/mpquic/repository/onsi/ginkgo\""))
	}
}
