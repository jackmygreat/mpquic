...
var _ = Describe("Subcommand", func() {
	Describe("ginkgo bootstrap", func() {
		...
		It("should generate a bootstrap file, as long as one does not exist", func() {
			...
			Ω(content).Should(ContainSubstring("\t" + `. "github.com/yyleeshine/mpquic/repository/onsi/ginkgo"`))
			Ω(content).Should(ContainSubstring("\t" + `. "github.com/yyleeshine/mpquic/repository/onsi/gomega"`))
		...
		It("should generate an agouti bootstrap file when told to", func() {
			...
			Ω(content).Should(ContainSubstring("\t" + `. "github.com/yyleeshine/mpquic/repository/onsi/ginkgo"`))
			Ω(content).Should(ContainSubstring("\t" + `. "github.com/yyleeshine/mpquic/repository/onsi/gomega"`))
			Ω(content).Should(ContainSubstring("\t" + `"github.com/yyleeshine/mpquic/repository/sclevine/agouti"`))
		})
	...
	Describe("ginkgo generate", func() {
		...
		Context("with nodot", func() {
			It("should not import ginkgo or gomega", func() {
				...
				Ω(err).ShouldNot(HaveOccurred())
				Ω(content).Should(ContainSubstring("package foo_bar_test"))
				Ω(content).ShouldNot(ContainSubstring("\t" + `. "github.com/yyleeshine/mpquic/repository/onsi/ginkgo"`))
				Ω(content).ShouldNot(ContainSubstring("\t" + `. "github.com/yyleeshine/mpquic/repository/onsi/gomega"`))
			})
		})
		...
		Context("with agouti", func() {
			It("should generate an agouti test file", func() {
				...
				Ω(content).Should(ContainSubstring("\t" + `. "github.com/yyleeshine/mpquic/repository/onsi/ginkgo"`))
				Ω(content).Should(ContainSubstring("\t" + `. "github.com/yyleeshine/mpquic/repository/onsi/gomega"`))
				Ω(content).Should(ContainSubstring("\t" + `. "github.com/yyleeshine/mpquic/repository/sclevine/agouti/matchers"`))
				Ω(content).Should(ContainSubstring("\t" + `"github.com/yyleeshine/mpquic/repository/sclevine/agouti"`))
