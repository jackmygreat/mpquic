...
type session struct {
	...
	pathTimers chan *path //在path当中传递，如果一个路径超时的话，就会传递给该session
...
runLoop:
	for {
		...
		s.maybeResetTimer() //每一轮都要看看要不要更新一下session的timer，这个timer主要是负责管理握手超时、解密超时、ping超时
...
func (s *session) handlePacketImpl(p *receivedPacket) error {// 处理session接收到的所有的报文，将这些报文散在对应的path上去，将其解密，path会将解密后的frame 发送到 handleFrames 处理
	...
	s.lastNetworkActivityTime = p.rcvTime //更新session的网络活动时间
...
func (s *session) handleStreamFrame(frame *wire.StreamFrame) error {
	...
	if frame.FinBit {
		...
		for pathID, pth := range s.paths {
			...
			fmt.Println("Path %x: sent %d retrans %d lost %d; rcv %d", pathID, sntPkts, sntRetrans, sntLost, rcvPkts)
		}
		s.pathsLock.RUnlock()
	}
...
func (s *session) GetVersion() protocol.VersionNumber { // 拿到该quic协议的版本
	return s.version
}
